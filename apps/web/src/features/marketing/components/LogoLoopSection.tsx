  ...
  return (
    <section id="technologies" className="w-full scroll-mt-32 px-4 py-20 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <div>
          <p className="font-mono text-sm font-semibold uppercase tracking-normal text-cyan-400">
            Nova Runtime
          </p>
          ...
          <div className="grid w-full max-w-[492px] grid-cols-3 justify-items-center gap-3 sm:grid-cols-4 lg:grid-cols-5">
            {stackIcons.map((item) => (
              <a
                ...
                className="group flex h-16 w-full max-w-16 cursor-pointer items-center justify-center rounded-2xl bg-white/[0.04] transition duration-300 hover:-translate-y-1 hover:bg-white/[0.08] sm:h-20 sm:max-w-20 lg:h-[88px] lg:max-w-[88px]"
              >
                <img
                  src={item.src}
                  alt={item.title}
                  className="h-7 w-7 object-contain transition duration-300 group-hover:scale-110 sm:h-9 sm:w-9 lg:h-10 lg:w-10"
                />
              </a>
            ))}
          </div>
